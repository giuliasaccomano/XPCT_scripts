...
input_dir = 'D:\\STP\\recons\\' 
#'E:\\Melanoma_may23\\recons\\B21-20177-01C\\B21_20177_01C_2um_dx\\' 

# export dir 
export_dir = 'E:\\LungTissue_nov23\\B023.017213-2A\\' 
...
range_slice = n_slice - 5 # generally a 20-30 range
...
def detect_rotation(I1,I2,display):
    ...
    if (display):
        fig, ax = plt.subplots(figsize=(19, 30), ncols=2, nrows=3)#pyplot.subplots(figsize=(19, 30), ncols=2, nrows=3)
        # display input data
        ax[0][0].imshow(I1,cmap='Greys_r')
        ax[0][0].set_title('I1')
        ax[0][1].imshow(I2,cmap='Greys_r')
        ax[0][1].set_title('I2')
        # map the auto-correlation functions to polar coordinates -> a rotation becomes a translation
        ax[1][0].imshow(I1_polar);
        #ax[1][0].imshow(ACF1_polar);
        ax[1][0].set_title('polar ACF1')
        #ax[1][1].imshow(ACF2_polar);
        ax[1][1].imshow(I2_polar);
        ax[1][1].set_title('polar ACF2')
        # display difference for validation
        ax[2][1].imshow(Diff);
        ax[2][1].set_title('difference after correction')
        ax[2][0].imshow(I1-I2);
        ax[2][0].set_title('difference without correction')
        plt.show()

        print('correlation between original images = ',corr(I1,I2))
        print('correlation after correction = ',_corr)
    else:
...
def detect_corr(img1,filenames_moving,overlap_range,cropped_rows_and_cols):
    ...
    fig,ax=plt.subplots(figsize=(19,10),ncols=2)
    ax[0].imshow(img1-img2, cmap = 'gray')
    ax[1].plot(np.linspace(0,len(res),len(res)),res[:,0],'b-')
    #plt.show()
    ...
    if compute_minmax == True:
        ...
        for i in range(0,len(dirnames)):
            if '' in dirnames[i]:
                ...
                filenames=np.sort(getListOfFiles(input_dir+dirnames[i]))
...
try:
    if not compute_minmax or (start_stitching and compute_minmax):
        ...
        if reverse_files:
            filenames_fixed=np.sort(getListOfFiles(input_dir+dirnames[0]))[::-1]
        else:
            filenames_fixed=np.sort(getListOfFiles(input_dir+dirnames[0]))
        
        if reverse_files:
            filenames_float=np.sort(getListOfFiles(input_dir+dirnames[1]))[::-1]
        else:
            filenames_float=np.sort(getListOfFiles(input_dir+dirnames[1]))
...
try:
    if not compute_minmax or (start_stitching and compute_minmax):
        ...
        for i in range(1,len(dirnames)):
            ...
            if reverse_files:
                filenames_moving=np.sort(getListOfFiles(input_dir+dirnames[i]))[::-1]
            else:
                filenames_moving=np.sort(getListOfFiles(input_dir+dirnames[i]))
