...
def create_export_dir(output_dir,export_data_type):
    """
    ...
    if export_data_type == 0:
        export_dir = os.path.join(output_dir,'slices8bit\\')
    elif export_data_type == 1:
        export_dir = os.path.join(output_dir,'slices16bit\\')
    else:
        export_dir = os.path.join(output_dir,'slices32bit\\')
...
def export_converted_images2(input_dir,output_dir,export_data_type,max_hist,min_hist):
    """
    Return THE END when it lasts running
    Par:
        - input_dir: a generic path to a folder with images inside
    ...
    """
    outdir = create_export_dir(output_dir,export_data_type)
    indir_slices = create_input_dir(input_dir)
    dirnames = get_filelist_in_dir(indir_slices)
    count = 0
    for entry in dirnames:
...
if single_stack == False:
    ...
    if indir1:
        export_converted_images2(indir1,output_dir,export_data_type,max_hist1,min_hist1)
    if indir2:
        export_converted_images2(indir2,output_dir,export_data_type,max_hist1,min_hist1)
    if indir3:
        export_converted_images2(indir3,output_dir,export_data_type,max_hist1,min_hist1)
